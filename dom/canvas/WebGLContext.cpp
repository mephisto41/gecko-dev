...
{
    int32_t status;
    if (!NS_SUCCEEDED(gfxUtils::GetFeatureStatus(gfxInfo, feature, &status)))
        return false;
...
HasAcceleratedLayers(const nsCOMPtr<nsIGfxInfo>& gfxInfo)
{
    ...
    gfxUtils::GetFeatureStatus(gfxInfo, nsIGfxInfo::FEATURE_DIRECT3D_9_LAYERS, &status);
    if (status)
        return true;
    gfxUtils::GetFeatureStatus(gfxInfo, nsIGfxInfo::FEATURE_DIRECT3D_10_LAYERS, &status);
    if (status)
        return true;
    gfxUtils::GetFeatureStatus(gfxInfo, nsIGfxInfo::FEATURE_DIRECT3D_10_1_LAYERS, &status);
    if (status)
        return true;
    gfxUtils::GetFeatureStatus(gfxInfo, nsIGfxInfo::FEATURE_DIRECT3D_11_LAYERS, &status);
    if (status)
        return true;
    gfxUtils::GetFeatureStatus(gfxInfo, nsIGfxInfo::FEATURE_OPENGL_LAYERS, &status);
    if (status)
        return true;
